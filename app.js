...
function sendToGAS(params){
  return new Promise((resolve,reject)=>{
    ...
    window[cb]=(res)=>{ cleanup(); if(res && res.ok===false){ reject(new Error(res.error||"server error")); } else { resolve(res); } };
...
function repFreqTable(pairs,total,col1){
  ...
  return "<table class='rep'><colgroup><col><col class='cn'><col class='cp'></colgroup><thead><tr><th>"+col1+"</th><th class='num'>n</th><th class='num'>%</th></tr></thead><tbody>"+body+totalRow+"</tbody></table>";
}
...
function completenessByFacilityHtml(rows){
  ...
  return "<table class='rep'><colgroup><col><col class='cn'><col class='cn'><col class='cn'><col class='cp'></colgroup><thead><tr><th>មូលដ្ឋានបញ្ជូន · Referring facility</th><th class='num'>សរុប</th><th class='num'>ពេញលេញ</th><th class='num'>មិនពេញលេញ</th><th class='num'>អត្រា · Rate</th></tr></thead><tbody>"+body+"</tbody></table>";
}
function buildReportHTML(){
  ...
  // Reuse the logo embedded once in the page header (no duplicated data URI)
  var _logoEl=document.getElementById("hospitalLogo");
  var logoImg=(_logoEl&&_logoEl.src)?"<img class='rep-logo' src='"+_logoEl.src+"' alt='KCPH logo'>":"";
